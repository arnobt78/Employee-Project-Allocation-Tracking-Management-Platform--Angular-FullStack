...
|---|---|---|---|
| **Google AI Studio (Gemini)** 🔑 | **Yes** — genuinely indefinite, no card, no expiration | `gemini-2.5-flash`, `gemini-2.5-flash-lite` (Pro tier is paid-only as of April 2026). Preview Flash-class IDs may also appear free — confirm in AI Studio. | OpenAI-compatible endpoint: `https://generativelanguage.googleapis.com/v1beta/openai/`. This repo uses native `generateContent`. Free-tier prompts may be used to improve Google's products. |
| **GroqCloud** 🔑 | **Yes** — no card | Production text: `openai/gpt-oss-120b`, `openai/gpt-oss-20b`; preview: `qwen/qwen3.6-27b`. **`llama-3.1-8b-instant` and `llama-3.3-70b-versatile` shut down for free/developer tiers on 2026-08-16.** | Extremely fast LPU inference. See [deprecation watch](#groq-deprecation-watch-verified-2026-08-24-primary-source). |
| **OpenRouter** 🔑 | **Yes** — no card for `:free` models | Models whose ID ends in `:free` only (list rotates). Prefer `openai/gpt-oss-120b:free`, `openai/gpt-oss-20b:free`, `google/gemma-4-31b-it:free`. Check `https://openrouter.ai/models?max_price=0`. | Append `:free` per OpenRouter docs. Typical free limits ~20 RPM / tens–hundreds RPD. DeepSeek `:free` IDs are often paid again — do not hardcode without checking. |
...
| **Hugging Face (Inference Providers / router)** 🔑 | **Yes** — no card for the free tier | Access to 15+ upstream providers through `https://router.huggingface.co/v1` | Free tier ≈ $0.10/month Inference Provider credits (PRO higher). Useful as last-rung fallback — expect some 404/410 as hosts rotate. |
...
Fetched / cross-checked from `https://console.groq.com/docs/deprecations` and current model catalog:

| Deprecated model | Shutdown date | Recommended replacement |
|---|---|---|
| `llama-3.1-8b-instant` | **2026-08-16 (past — free/developer tiers)** | `openai/gpt-oss-20b` |
| `llama-3.3-70b-versatile` | **2026-08-16 (past — free/developer tiers)** | `openai/gpt-oss-120b` or `qwen/qwen3.6-27b` |
...
**This repo:** `api/employee-management/ai-providers.mjs` uses `openai/gpt-oss-120b` → `openai/gpt-oss-20b` → `qwen/qwen3.6-27b` for Groq. Do not reintroduce `llama-3.1-8b-instant` on free keys.
...
This repository (EmpowerHub — Angular SPA + Vercel serverless API) ships a working, minimal implementation of exactly this pattern at `api/employee-management/`:

- **`ai-providers.mjs`** — Layer 1 registry + Layer 2 clients (Gemini native `generateContent` + OpenAI-compatible `/chat/completions`) + Layer 3 orchestrator (`completeChatWithFallback`). Providers in order: Gemini → Groq → OpenRouter (`:free`) → Hugging Face router. Env keys are **server-only** (`GOOGLE_GEMINI_API_KEY`, `GROQ_LLAMA_API_KEY`, `OPENROUTER_API_KEY`, `HUGGINGFACE_API_KEY` / `HF_TOKEN`) — never `NG_APP_*`.
- **`repository.mjs` → `generateOverviewDraft`** — builds the overview prompt, calls `completeChatWithFallback`, parses JSON into the project overview DTO.

It intentionally does **not** wire Cloudflare Workers AI (needs an account ID) or speech/image/embeddings — out of scope for overview drafting. Treat `ai-providers.mjs` as the reusable shape (registry + generic client + orchestrator); swap model rows when free tiers change.

Copy env names and signup links from the root **`.env.example`**.
